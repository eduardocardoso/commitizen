...
class Changelog:
    ...
    def __init__(self, config: BaseConfig, args):
        ...
        self.merge_tags = args.get("merge_tags") or self.config.settings.get(
            "merge_tags"
        )
            ...
            for entry in tree:
                if single_entry is None:
                    ...
                    for change_type in entry["changes"]:
                        single_entry["changes"][change_type] = single_entry[
                            "changes"
                        ].get(change_type, [])
                        single_entry["changes"][change_type] += entry["changes"][
                            change_type
                        ]
            tree = [single_entry]
